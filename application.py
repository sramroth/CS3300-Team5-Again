        ...
        with open('assignments.txt', 'w') as assignments_file:
            for course in self.course_list.course_list:
                for assignment in course.assignment_list.assignment_list:
                    assignments_file.write(assignment.course + ' ' + assignment.name + ' ' + assignment.due_date + '\n')
    ...
    def __init__(self, parent, course_list):
        ...
        for course in course_list.course_list:
            ...
            for assignment in course.assignment_list.assignment_list:
                ...
                # Assignment Due Date
                due_label = Label(assignment_frame, text="Due: " + assignment.due_date, font=("Verdana", 8, "bold"),
                                  relief=GROOVE, anchor=W)
...
class AddAssignmentPage(tk.Frame):
    def __init__(self, parent, course_list):
        ...
        self.assignment_due_entry_label = Label(self, text="Enter the due date (MM/DD/YYYY):", font=("Verdana", 10))
        self.assignment_due_entry_label.pack(padx=3, pady=3)

        self.assignment_due_entry = Entry(self)
        self.assignment_due_entry.pack(padx=3, pady=3)
    ...
    def add_assignment_button_command(self, course_list):
        assignment = Assignment(self.assignment_name_entry.get(),
                                self.course_selection.get(), self.assignment_due_entry.get())
